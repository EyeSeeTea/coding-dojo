...
function hasValidStatus(status: string): status is RequestStatus {
    return ["pending", "success", "error"].includes(status);
}

function getRequestFromString(value: string): Either<Error, Request> {
    const parsedJSON = JSON.parse(value);
    if (typeof parsedJSON !== "object" || parsedJSON === null || !hasValidStatus(parsedJSON.status))
        return Either.error(new Error("Invalid JSON"));
