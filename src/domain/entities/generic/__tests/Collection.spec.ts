...
describe("Collection", () => {
    test.only("indexOf", () => {
        const indexOne = _(["a", "b", "c"]).indexOf("b");
        expect(indexOne).toEqual(1);

        const indexUndefined = _(["a", "b", "c"]).indexOf("d");
        expect(indexUndefined).toBeUndefined();

        const indexNumber = _([1, 2, 3]).indexOf(1);
        expect(indexNumber).toBe(0);
    });
